...
bool FMap::ReadHeader(std::ifstream &stream) {
	std::string line;
	mapWidth = -1;
	mapHeight = -1;
...
bool FMap::ReadLayerData(std::ifstream &stream) {
	std::string line;
...
bool FMap::ReadEntityData(std::ifstream &stream) {
	std::string line;
	std::string type;
	while(getline(stream, line)) {
		if(line == "") { break; }
...
void FMap::Load(const std::string fileName) {
